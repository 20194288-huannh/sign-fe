...
contract BlockSig is Ownable {
    ...
    function createDoc(bytes32 hash, address[] memory signers, bool notarize) onlyOwner public {
        ...
        if (signers.length == 0 && !notarize){
            documents[hash].createdTime = block.timestamp;
            documents[hash].remainingSignatures = 0;
            documents[hash].completedTime = block.timestamp;
            documents[hash].status = currentState.signed;
            finalDocs[hash] = hash;
        } else {
            documents[hash].createdTime = block.timestamp;
            documents[hash].remainingSignatures = signers.length;
            documents[hash].completedTime = 0;
            documents[hash].completedHash = "";
            documents[hash].status = currentState.pending;
    ...
    function finalizeDoc(bytes32 originalHash, bytes32 finalHash) onlyOwner public {
        ...
        documents[originalHash].completedTime = block.timestamp;
    }
